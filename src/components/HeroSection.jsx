  ...
  return (
    <div className="relative flex-1" ref={ref}>
      <button
        ...
        className="w-full flex items-center justify-between bg-white/95 backdrop-blur-sm text-gray-700 px-5 py-3.5 rounded-xl text-sm font-medium hover:bg-white transition-all duration-300 border border-transparent hover:border-[#31b56b]/30 shadow-sm"
      >
        <div className="flex items-center gap-2">
          {Icon && <Icon className="w-4 h-4 text-[#31b56b]" />}
          <span className={value ? 'text-[#0f172a] font-bold' : 'text-gray-400 font-medium'}>
            {value || label}
          </span>
        </div>
        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform duration-300 ${open ? 'rotate-180 text-[#31b56b]' : ''}`} />
      </button>
            ...
            {options.map((opt) => (
              <button
                ...
                className={`w-full text-left px-5 py-3 text-sm transition-all duration-200
                  ${value === opt
                    ? 'bg-[#31b56b]/10 text-[#31b56b] font-bold border-l-4 border-[#31b56b]'
    ...
    <section className="relative w-full h-[750px] md:h-[850px] z-30">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full px-4 pt-20">
        <div className="max-w-4xl w-full text-center space-y-6">
          <div className="inline-block px-4 py-1.5 bg-[#31b56b]/20 backdrop-blur-md rounded-full border border-[#31b56b]/30 mb-4 animate-pulse">
            <span className="text-[#31b56b] text-xs font-bold tracking-[0.2em] uppercase">Premium Car Experience</span>
          </div>
          ...
          >
            DISCOVER YOUR <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#31b56b] to-[#4ade80]">DREAM CAR</span> AT MOTOR
          </h1>
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex gap-3 flex-[1.5]">
                <div className="relative flex-1 group">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-[#31b56b] transition-colors" />
                  <input
                    type="text"
                    ...
                    className="w-full bg-white/95 backdrop-blur-sm text-[#0f172a] pl-11 pr-4 py-3.5 rounded-xl text-sm placeholder-gray-400 outline-none border border-transparent focus:border-[#31b56b]/50 shadow-sm transition-all"
                  />
                </div>
                <button
                  onClick={handleSearch}
                  className="bg-[#31b56b] text-white px-8 py-3.5 rounded-xl text-sm font-bold hover:bg-[#28a05e] active:scale-95 transition-all duration-300 shadow-lg shadow-[#31b56b]/25 flex items-center justify-center gap-2 uppercase tracking-wider"
                >
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce opacity-50">
        <div className="w-1 h-12 rounded-full bg-gradient-to-b from-[#31b56b] to-transparent" />
      </div>
    </section>
  );
}
